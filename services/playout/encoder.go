...
// errEncoderExited is returned by Write when the ffmpeg process has exited, so
// the pacing loop reconnects instead of blocking on a dead pipe.
var errEncoderExited = errors.New("encoder process exited")
...
type encoder struct {
	...
	stopOnce sync.Once // Stop is idempotent — the reconnect path may Stop a dead encoder the defer also reaps
...
func startEncoder(c encoderConfig) (*encoder, error) {
	...
	if c.FPS <= 0 {
		c.FPS = config.DefaultVideoFPS
	}
	if c.Width <= 0 {
		c.Width = config.DefaultVideoWidth
	}
	if c.Height <= 0 {
		c.Height = config.DefaultVideoHeight
	}
	if c.AudioBitrate == "" {
		c.AudioBitrate = config.DefaultAudioBitrate
	}
	if c.FontFile == "" {
...
func (e *encoder) Write(p []byte) error {
	...
	// transient muxer starvation the buffer exists to ride through. Abandon the
	// send the moment the process exits (done closed) so a dead encoder surfaces
	// as an error instantly instead of blocking the pacing loop on a dead pipe
	// until the OS reports the broken write (seconds on Windows).
	select {
	case e.stdinCh <- p:
		return nil
	case <-e.done:
